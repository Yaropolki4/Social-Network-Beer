...
class Chat(db.Model):
    ...
    def get_chat(user_1: int, user_2: int):
        chat = Chat.query.filter_by(user_one_id=user_1, user_two_id=user_2).first()
        if chat:
            return chat
        else:
            chat = Chat.query.filter_by(user_one_id=user_2, user_two_id=user_1).first()
            print(1)
            if chat:
                return chat
            else:
...
class Messages(db.Model):
    ...
    def get_messages_for_both(chat_id, user_1, user_2) -> dict:
        print(user_1, user_2)
        messages = Messages.query.filter(Messages.chat_id==chat_id, Messages.user_id.in_([user_1, user_2])).\
